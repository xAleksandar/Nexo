...
contract Lottery is ReentrancyGuard {
    ...
    uint start_block;                       // At this block users can start buying tickets. Enter 0 to start from the current block.
    uint end_block;                         // Users should not be able to buy tickets after this block.
    ...
    function buyTicket(string memory _externalStoragePointer) public payable nonReentrant {
        require(block.number >= start_block,   "Tickets cannot be buyght yet.");
        require(block.number <= end_block,     "Tickets cannot be buyght anymore.");
        require(msg.value >= 1000000000000000, "Ticket Price is 0.001 ETH.");
        if (internalStorage.nftContractsBook(msg.sender) == address(0)) {
            // Notice 111 used as create2 salt.
            BeaconProxy nftContract = BeaconProxy(payable(factory.create2NFTContract(toHexString(msg.sender), "LOTTERY", 111)));
        ...
        uint nftId = nftProxy.mint(_externalStoragePointer, msg.sender);
        ...
        if (block.number >= start_block + ((end_block - start_block)/2) && !FirstWinnerSelected) {
    ...
    function sendMoney (address _to, uint value) internal {
    ...
    */
    ...
    function pickSurpriseWinner(address _winner) internal {
        if (
            ...
            (uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, getTicketsCount())))%getTicketsCount() + 1)%1 == 0 
        ) {
    ...
    */
    ...
    function pickFinalWinner() public payable nonReentrant returns (address) {
        ...
        uint ticketId = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, getTicketsCount())))%getTicketsCount() + 1;
        address winner = NFT(internalStorage.getNFTproxy(ticketId)).ownerOf(internalStorage.getNFTid(ticketId));
